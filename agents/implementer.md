...
If the field says `none`, the planner looked and found nothing that applies — which is what every milestone says in a project that ships no skills. What `none` is not is the field being ABSENT, or present with nothing after the colon: neither can be told apart from a planner that never looked, and you load nothing in all three cases. The reviewer checks that on every plan; `spec-trace` only fails a milestone over it where the project set `trace.require_skills_field`, which most do not. So a milestone that reaches you without the field has passed nothing that would have caught it — say so in `NOTES:` rather than assuming the answer was `none`.
...
The orchestrator routes this to the `architect` and re-invokes you with `ARCHITECT_GUIDANCE`. Then implement per that guidance.
